...
exports.handler = function(event, context, callback) {
  ...
  var key;
  if (event.queryStringParameters && event.queryStringParameters.key && event.queryStringParameters.key !== "") {
    console.log("Call from API Gateway.");
    key = event.queryStringParameters.key;
  } else if (event.Records && event.Records !== ""){
    ...
    key = '800x600/' + event.Records[0].s3.object.key;
  } else {
